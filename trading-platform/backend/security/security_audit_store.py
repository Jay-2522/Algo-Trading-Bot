...
class SecurityAuditStore:
    """In-memory security audit event store."""

    _events: list[dict] = []

    def store_event(self, event: dict) -> dict:
        stored = {
            "event_id": event.get("event_id") or f"security_audit_{uuid4().hex[:12]}",
            "event_type": event.get("event_type", "SECURITY_CHECK"),
            "message": event.get("message", "Security audit event recorded."),
