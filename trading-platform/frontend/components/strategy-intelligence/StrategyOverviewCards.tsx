...
export function StrategyOverviewCards({ overview }: { overview: StrategyOverview }) {
  const cards = [
    ...
    ["Avg Confidence", `${Number(overview.avg_confidence || 0).toFixed(2)}%`],
    ["Avg Risk Efficiency", `${Number(overview.avg_risk_efficiency || 0).toFixed(2)}%`],
    ["Avg Execution Efficiency", `${Number(overview.avg_execution_efficiency || 0).toFixed(2)}%`],
    ["Top Ranked Strategy", overview.top_ranked_strategy || "Pending"],
    ["Session Efficiency", `${Number(overview.session_efficiency || 0).toFixed(2)}%`],
  ];
